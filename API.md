...
上报 `identify_id` 对应识别结果是否正确。接口与识别接口使用相同的 API Key；重复提交相同结果是幂等的。`correct` 为 `false` 时，服务端会保存原图并将该记录放入用户训练标注页的“领取平台待标注”队列；每次最多领取 10 张，领取后 30 分钟内未提交会自动回到队列，刷新同一浏览器会恢复该领取批次。标注提交后直接进入待训练数据，不进入管理员审核。`correct` 为 `true` 时只记录反馈并删除临时原图；未反馈记录不会进入标注、审核或训练流程，并在 60 秒后清理。
