...
#loginView{min-height:100vh;display:grid;place-items:center;padding:18px}.login{width:min(420px,100%);background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:20px;box-shadow:0 20px 60px rgba(0,0,0,.35)}.login h1{margin:0 0 10px;font-size:20px}.login input{width:100%;margin:10px 0}.login button{width:100%}#appView{display:none}header{position:sticky;top:0;z-index:20;background:#161b22;border-bottom:1px solid var(--line);display:flex;gap:10px;align-items:center;padding:8px 12px}.brand{font-weight:800;font-size:16px}.spacer{flex:1}.topSwitch{display:flex;align-items:center;gap:8px;padding:7px 10px;border:1px solid var(--line);border-radius:999px;background:#0d1117;color:#c9d1d9;white-space:nowrap}.topSwitch input{min-height:auto;width:16px;height:16px;accent-color:#238636}main{padding:10px;display:grid;gap:10px}.stats{display:grid;grid-template-columns:repeat(4,minmax(100px,1fr));gap:8px}.stat,.panel,.item{background:var(--panel);border:1px solid var(--line);border-radius:10px}.stat{padding:9px 11px}.stat b{display:block;font-size:22px}.panel{padding:10px}.row{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.list{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:10px}.item{padding:9px}.captchaPreview{position:relative;height:178px;background:#05080c;border-radius:8px;overflow:hidden}.captchaPreview img{display:block;width:100%;height:100%;object-fit:contain;cursor:zoom-in}.imageSelections{position:absolute;inset:0;pointer-events:none}.imageSelection{position:absolute;border:3px solid;box-shadow:0 0 0 1px rgba(0,0,0,.7);font-size:11px;font-weight:700;line-height:1}.imageSelection.user{border-color:#3fb950}.imageSelection.model{border-color:#58a6ff;border-style:dashed}.imageSelection .selectionTag{position:absolute;padding:3px 5px;border-radius:4px;color:#fff;text-shadow:0 1px 1px #000;white-space:nowrap}.imageSelection.user .selectionTag{left:-3px;top:-3px;background:#238636}.imageSelection.model .selectionTag{right:-3px;bottom:-3px;background:#1f6feb}.selectionLegend{display:flex;gap:8px;flex-wrap:wrap;margin:6px 0 2px;font-size:12px}.selectionLegend span{display:inline-flex;align-items:center;gap:4px}.selectionLegend i{display:inline-block;width:12px;height:12px;border:2px solid}.selectionLegend .userMark{border-color:#3fb950}.selectionLegend .modelMark{border-color:#58a6ff;border-style:dashed}.badge{display:inline-block;padding:2px 7px;border-radius:999px;background:#334155}.badge.needs_admin{background:#854d0e}.badge.approved{background:#166534}.badge.rejected,.badge.auto_rejected{background:#991b1b}.badge.screening{background:#1d4ed8}.small{font-size:12px}.item input{width:92px}.item textarea{width:100%;min-height:54px;margin-top:6px;background:#0d1117}.status{white-space:pre-wrap;background:#0d1117;border:1px solid var(--line);border-radius:8px;padding:8px;color:var(--muted);max-height:220px;overflow:auto}#zoom{position:fixed;inset:0;z-index:50;background:rgba(0,0,0,.86);display:none;align-items:center;justify-content:center;padding:22px}#zoom img{max-width:96vw;max-height:92vh;border-radius:10px;background:#05080c}#zoom button{position:fixed;right:18px;top:14px}@media(max-width:800px){.stats{grid-template-columns:repeat(2,1fr)}.list{grid-template-columns:1fr}header{flex-wrap:wrap}}
</style></head><body>
...
<section id="appView"><header><span class="brand">管理员审核</span><button id="refreshBtn">刷新</button><label>状态 <select id="statusFilter"><option value="review">待审核</option><option value="in_progress">处理中</option><option value="ready">待训练</option><option value="rejected">已拒绝</option><option value="all">全部</option></select></label><label class="topSwitch" title="训练时是否包含模型和用户一致后自动通过的样本"><input id="includeAutoApproved" type="checkbox" checked> 训练包含自动通过</label><span class="spacer"></span><button id="logoutBtn">退出</button><a href="/">用户页</a></header><main><section class="stats" id="stats"></section><section class="panel"><div class="row"><b>Key 有效期</b><label>无任务默认 <input id="noTaskRewardHours" type="number" min="0.25" max="720" step="0.25" style="width:90px"> 小时</label><label>答题奖励 <input id="answerRewardHours" type="number" min="0.25" max="720" step="0.25" style="width:90px"> 小时</label><button id="saveRewardConfigBtn" class="primary">保存有效期</button><span id="rewardConfigStatus" class="muted"></span></div><p class="small muted">保存后仅影响新生成的 Key；可设置范围为 0.25 至 720 小时（30 天）。</p></section><section class="panel"><div class="row"><label>run-name <input id="runName" value="web_review_parts_v1" style="width:170px"></label><label>feature <select id="featureMode"><option value="parts_v1">parts_v1</option><option value="processed_v2">processed_v2</option></select></label><label>epochs <input id="epochs" type="number" min="1" value="180" style="width:90px"></label><label>val <input id="valRatio" type="number" min="0.05" max="0.9" step="0.05" value="0.2" style="width:80px"></label><button id="trainBtn" class="success">全部审核后训练</button><button id="trainStatusBtn">训练状态</button></div><div id="trainStatus" class="status">尚未查询。</div></section><section class="list" id="list"></section></main></section><div id="zoom"><button id="zoomClose">关闭 Esc</button><img id="zoomImg" alt="放大预览"></div>
<script>
const KEY='sameobject_admin_key'; const $=id=>document.getElementById(id); let adminKey=sessionStorage.getItem(KEY)||''; function esc(s){return String(s??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));} function headers(){return {'Content-Type':'application/json','X-Admin-Key':adminKey};} function formatHours(seconds){const hours=Number(seconds)/3600;return Number.isInteger(hours)?String(hours):String(hours).replace(/\.0+$/,'');} async function api(url,opts={}){opts.headers={...(opts.headers||{}),...headers()};const r=await fetch(url,opts);const d=await r.json().catch(()=>({}));if(!r.ok||d.code){if(r.status===401)showLogin('Key 不正确或已失效。');throw new Error(d.message||r.statusText);}return d.data??d;} function showLogin(msg=''){ $('loginView').style.display='grid'; $('appView').style.display='none'; $('loginMsg').textContent=msg; } async function loadConfig(){try{const r=await fetch('/api/config'); const d=await r.json(),c=d.data||{}; if(typeof c.include_auto_approved_in_training==='boolean') $('includeAutoApproved').checked=c.include_auto_approved_in_training; $('noTaskRewardHours').value=formatHours(c.no_task_api_key_ttl_seconds); $('answerRewardHours').value=formatHours(c.answer_api_key_ttl_seconds);}catch(e){}} async function showApp(){ $('loginView').style.display='none'; $('appView').style.display='block'; await loadConfig(); loadList().catch(e=>showLogin(e.message)); }
...
const adminItems=new Map(); const statusLabels={needs_admin:'待审核',uploaded:'待标注',labeling:'标注中',queued:'待初筛',screening:'初筛中',approved:'待训练',auto_rejected:'自动拦截',rejected:'已拒绝'}; function badge(s){return `<span class="badge ${esc(s)}">${esc(statusLabels[s]||s)}</span>`;} function prediction(it){const result=it.screen||it.platform_feedback||{}; return {pair:Array.isArray(result.pred_pair)?result.pred_pair:[],animal:result.pred_animal||'',confidence:result.confidence,reason:result.reason||''};} function validPair(pair){return pair.filter(n=>Number.isInteger(Number(n))&&Number(n)>=1&&Number(n)<=8).map(Number);} function drawSelections(id){const it=adminItems.get(id), preview=document.querySelector(`[data-preview="${CSS.escape(id)}"]`); if(!it||!preview||!it.size?.[0]||!it.size?.[1])return; const image=preview.querySelector('img'), layer=preview.querySelector('.imageSelections'); if(!image.complete||!image.naturalWidth)return; const width=image.clientWidth,height=image.clientHeight,ratio=it.size[0]/it.size[1],shownWidth=Math.min(width,height*ratio),shownHeight=shownWidth/ratio,left=(width-shownWidth)/2,top=(height-shownHeight)/2; const add=(n,type,label)=>{const box=it.boxes?.[n-1];if(!box)return; const [x,y,w,h]=box,el=document.createElement('div'); el.className='imageSelection '+type; el.style.left=(left+x/it.size[0]*shownWidth)+'px';el.style.top=(top+y/it.size[1]*shownHeight)+'px';el.style.width=(w/it.size[0]*shownWidth)+'px';el.style.height=(h/it.size[1]*shownHeight)+'px';el.innerHTML=`<span class="selectionTag">${esc(label)} #${n}</span>`;layer.appendChild(el);}; layer.replaceChildren(); validPair(it.pair||[]).forEach(n=>add(n,'user','用户')); validPair(prediction(it).pair).forEach(n=>add(n,'model','识别'));} function drawAllSelections(){adminItems.forEach((_,id)=>drawSelections(id));} async function loadStats(){const d=await api('/api/admin/stats'), count=(...statuses)=>statuses.reduce((total,status)=>total+(d.counts[status]||0),0), stats=[['待审核',count('needs_admin')],['处理中',count('uploaded','labeling','queued','screening')],['待训练',count('approved')],['已拒绝',count('auto_rejected','rejected')]]; $('stats').innerHTML=stats.map(([label,total])=>`<div class="stat"><span class="muted">${label}</span><b>${total}</b></div>`).join('');} async function loadList(){await loadStats(); const st=$('statusFilter').value; const d=await api('/api/admin/items?status='+encodeURIComponent(st)); adminItems.clear(); d.items.forEach(it=>adminItems.set(it.id,it)); $('list').innerHTML=d.items.map(renderItem).join(''); document.querySelectorAll('[data-action]').forEach(b=>b.onclick=()=>review(b.dataset.id,b.dataset.action)); document.querySelectorAll('[data-zoom]').forEach(img=>img.onclick=()=>zoom(img.src)); document.querySelectorAll('[data-preview]').forEach(preview=>{const image=preview.querySelector('img'); image.onload=()=>drawSelections(preview.dataset.preview); if(image.complete)drawSelections(preview.dataset.preview);});}
function renderItem(it){const p=prediction(it),pair=(it.pair||[]).join(','),predPair=p.pair.join(','),hasPrediction=p.pair.length>0; return `<article class="item"><div class="row" style="justify-content:space-between"><b title="${esc(it.filename)}">${esc(it.filename)}</b>${badge(it.status)}</div><div class="captchaPreview" data-preview="${esc(it.id)}"><img data-zoom src="${it.image_url}" alt="审核图片 ${esc(it.filename)}" loading="lazy"><div class="imageSelections" aria-hidden="true"></div></div><div class="selectionLegend"><span><i class="userMark"></i>用户 [${esc(pair)||'未标注'}] ${esc(it.animal||'')}</span>${hasPrediction?`<span><i class="modelMark"></i>识别 [${esc(predPair)}] ${esc(p.animal)}</span>`:''}</div><div class="small muted">来源：${esc(it.source||'user_upload')} · 状态：${esc(statusLabels[it.status]||it.status)} · 创建：${esc(it.created_at||'')} ${it.submitted_at?'· 提交：'+esc(it.submitted_at):''}</div><div class="small ${it.screen?.ok===false?'bad':'yellow'}">${esc(p.reason||'无初筛信息')} ${hasPrediction?`预测=[${esc(predPair)}] ${esc(p.animal)} score=${esc(p.confidence)}`:''}</div><div class="row"><label>pair <input id="pair-${esc(it.id)}" value="${esc(pair)}"></label><label>animal <input id="animal-${esc(it.id)}" value="${esc(it.animal||'')}"></label></div><textarea id="note-${esc(it.id)}" placeholder="备注">${esc(it.review_note||'')}</textarea><div class="row"><button class="success" data-action="approve" data-id="${esc(it.id)}">通过</button><button class="danger" data-action="reject" data-id="${esc(it.id)}">拒绝</button><button class="warnBtn" data-action="needs_admin" data-id="${esc(it.id)}">待审</button></div></article>`;}
...
class TrainingWebHandler(BaseHTTPRequestHandler):
    ...
    def handle_admin_items(self, parsed) -> None:
        ...
        if status in status_groups:
            ...
            raise ValueError('status 必须是 review/in_progress/ready/rejected/all。')
